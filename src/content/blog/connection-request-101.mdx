...
- **Fleets of accounts.** One account can only send 15 to 20 invites a day safely. Scaling past that means running many in parallel.
- **Proxies.** Each account needs its own home-looking internet connection. Cheap data-centre IPs get flagged on sight.
- **Fingerprints.** Each account has to pretend to be the same specific computer every time it appears. Same browser version, screen size, time zone.
- **Warming.** New accounts can't just start firing invites. They need a slow ramp of normal browsing first.
- **Impossible travel.** If "your" account suddenly appears in London, Berlin, and São Paulo on the same day, it gets restricted.

If your automation tool is misbehaving and you've ruled out the endpoint-level basics, the cause is almost always one of those five.
