...
## "But I ran it in Claude Code and it still took a while?"

Yes. When I recorded the demo, Claude narrated between stages, and each bit of narration is Claude thinking, which takes seconds. Fair. But notice what was slow: the *talking*, not the *pipeline*. The data never touched Claude. And the whole point is you only need Claude in the room the first time. The finished command runs forever without it.

Compare that to the MCP version, where the slow expensive talking IS the workflow, unavoidably, on every single run, forever.
